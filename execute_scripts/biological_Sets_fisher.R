...
temp <- lapply(gwaslist, function(mydf){
  mydf$tissue <- gsub("_no_meta_analysis","",mydf$model_ID)
  mydf$p <- mydf$pvalue
  return(mydf)
})
...
save(gwaslist, file = paste0(getwd(), "/Results/gwaslist.RData"))
...
load(paste0(getwd(), "/Results/gwaslist.RData"))
source("/sc/arion/projects/va-biobank/marios/Clinical.Significance/Scripts/GSEA_ORplot_Functions.R")
source("/hpc/users/anyfam01/Global.Scripts/Global.Source.R")
library(ggplot2)
...
invisible(lapply(mylist, function(thislist) lapply(thislist, function(thisdf) if(all(is.na(thisdf$gene))) stop('There are NAs') else message('All good here'))))
...
# Extract genes only from each table
temp <- lapply(names(mylist), function(name){
  thispval <- mylist[[name]]
  thispval <- lapply(thispval, function(thisdf){
    #browser()
...
gsea.path = paste0(getwd(), "/Results/all_traits_V5") # all_combinations_TWAS_GSEA_fisher_V3 uses protein specific background for Bulk and has improved aesthetics
gsea.path = paste0(getwd(),"/Results/fisher/MGIPhenotypes_v4_protein_coding")
